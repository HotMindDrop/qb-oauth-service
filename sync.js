...
async function walkDrive(folderId, pathParts = []) {
  ...
  console.log(`🔎 Scanning folder: ${currentPath} (${files.length} items)`);
  const result = [];
...
(async () => {
  ...
  let downloadCount = 0;
  const totalToDownload = photoFiles.length;
  ...
  for (const file of photoFiles) {
    ...
    downloadCount++;
    console.log(`⬇️ Downloading (${downloadCount} of ${totalToDownload}): ${file.filename}`);
